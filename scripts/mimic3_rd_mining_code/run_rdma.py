#!/usr/bin/env python3
"""
Run the RDMA rare-disease extraction pipeline on the MIMIC-III re-annotated
rare-disease mining benchmark.

Usage (from RDMA repo root):
  python scripts/run_mimic3_rd_mining.py --mimic3_root /path/to/mimic-iii/1.4
"""
...
def main():
    parser = argparse.ArgumentParser(
        description="RDMA pipeline on MIMIC-III rare-disease mining benchmark"
    )
    parser.add_argument(
        "--mimic3_root",
        type=str,
    ...
    parser.add_argument(
        ...
        help=(
            "Output JSONL path "
            "(default: <results_dir>/mimic3_rd_mining/<model_type>_predictions.jsonl)"
        ),
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        help=("Path to abbreviations embeddings .npy file " "(default: %(default)s)"),
    )
    parser.add_argument(
        "--use_abbreviations",
        action="store_true",
        default=True,
        help="Enable abbreviation resolution in the multi-stage verifier (default: True)",
    )
    parser.add_argument(
        "--strict",
        action="store_true",
    ...
    output = args.output or (
        _RESULTS_DIR / "mimic3_rd_mining" / f"{args.model_type}_predictions.jsonl"
    )
    ...
    try:
        ...
        for i, sample in enumerate(
            tqdm(run_samples, total=len(run_samples), desc="MIMIC3-RD")
        ):
            try:
            ...
            try:
                ...
                if args.debug:
                    ts(f"  [{note_id}] verified  {len(verified)}")
