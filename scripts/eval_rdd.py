...
def llm_match_pairs(llm_client, pairs):
    ...
    for pred, gold in tqdm(pairs, desc="LLM matching"):
        ...
        if resp.upper() not in ("YES", "NO"):
            ts(
                f"  WARN unexpected LLM response for "
                f"({pred!r}, {gold!r}): {resp!r}"
            )
...
def score_document(pred_set, gold_set, llm_results):
    ...
    matched_pairs = [
        {"predicted": p, "gold": p, "match_type": "exact"}
        ...
        for g in sorted(unmatched_gold):
            if g not in matched_gold and llm_results.get((p, g), False):
                matched_pairs.append(
                    {"predicted": p, "gold": g, "match_type": "llm"}
                )
                matched_pred.add(p)
                matched_gold.add(g)
                break
...
def parse_args():
    ...
    parser.add_argument(
        "--approach",
        choices=["rdma", "zeroshot"],
        default="rdma",
        help="Prediction approach: 'rdma' (default) or 'zeroshot'",
    )
    parser.add_argument(
        "--gpu_id",
        type=int,
...
def main():
    ...
    prefix = "zeroshot_" if args.approach == "zeroshot" else ""
    task_suffix = "_rel" if args.task == "relation" else ""

    predictions_file = (
        _RESULTS_DIR / "rdd"
        / f"{prefix}{args.model_type}{task_suffix}_predictions.jsonl"
    )
    eval_output = (
        _RESULTS_DIR / "rdd"
        / f"eval_{prefix}{args.model_type}{task_suffix}.json"
    )
    ...
    if args.task == "relation":
        ...
        if all_pairs:
            ...
            ts(
                f"  LLM judged {yes_count}/{len(all_pairs)} pairs "
                "as equivalent"
            )
