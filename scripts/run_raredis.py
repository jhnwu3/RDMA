...
def main():
    ...
    parser.add_argument(
        ...
        type=Path,
        default=None,
        help="Output JSONL path (default: <results_dir>/raredis/<model_type>_predictions.jsonl)",
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--strict",
        action="store_true",
        help="Enable strict ORPHA-matching in the verifier (default: lax/best-judgement mode)",
    )
    parser.add_argument(
        "--exact_match",
        action="store_true",
    ...
    try:
        ...
        for i, sample in enumerate(tqdm(run_samples, total=len(run_samples), desc="RareDis")):
            try:
