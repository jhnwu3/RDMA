...
class MedCPTModel(EmbeddingModel):
    ...
    def __init__(self, device=None):
        ...
        self.doc_tokenizer = AutoTokenizer.from_pretrained("ncbi/MedCPT-Article-Encoder")
...
class SentenceTransformerEmbedModel(EmbeddingModel):
    ...
    def __init__(self, model_name: Optional[str], device: str = "cpu"):
        ...
        self.device = device
        print(f"Initializing SentenceTransformer with model: {model_name} on device: {device}")
        ...
        try:
            ...
            self._dimension = len(sample_embedding)
            print(f"Model initialized successfully. Embedding dimension: {self._dimension}")
...
class EmbeddingsManager:
    ...
    def __init__(self, model_type: str, model_name: Optional[str] = None, device: Optional[str] = None):
        """
        ...
        if model_type not in self.MODEL_TYPES:
            raise ValueError(f"Unsupported model type. Must be one of {list(self.MODEL_TYPES.keys())}")
        ...
        if model_type == 'medcpt':
            ...
            self.model = self.MODEL_TYPES[model_type](device=device if device else "cpu")
        else:
    ...
    def prepare_embeddings(self, embedded_documents: List[Dict[str, Any]]) -> np.ndarray:
    ...
    def search(self, query: Union[str, np.ndarray], index: faiss.Index, k: int = 5) -> Tuple[np.ndarray, np.ndarray]:
        """
