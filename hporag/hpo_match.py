...
class BaseHPOMatcher(ABC):
    ...
    @abstractmethod
    def match_hpo_terms(self, entities: List[str], metadata: List[Dict]) -> List[Tuple[str, str]]:
        """Match entities to HPO terms."""
        pass

    @abstractmethod
    def process_batch(self, entities_batch: List[List[str]], metadata_batch: List[List[Dict]]) -> List[List[Tuple[str, str]]]:
...
class RAGHPOMatcher(BaseHPOMatcher):
    ...
    def _retrieve_candidates(self, entity: str) -> List[Dict]:
        ...
        distances, indices = self.embeddings_manager.search(query_vector, self.index, k=100)
    ...
    def _enrich_metadata(self, entity: str, candidates: List[Dict]) -> List[Dict]:
        ...
        for candidate in candidates:
            ...
            if cleaned_term in cleaned_phrase or cleaned_phrase in cleaned_term:
                ...
                score = min(len(cleaned_term), len(cleaned_phrase)) / max(len(cleaned_term), len(cleaned_phrase))
                add_candidate(
                    {'info': term, 'hp_id': hp_id},
                    score,
                    'substring'
    ...
    def match_hpo_terms(self, entities: List[str], metadata: List[Dict], original_sentences: Optional[List[str]] = None) -> List[Dict]:
        ...
        for entity, original_sentence in zip(entities, original_sentences):
            ...
            match_info = {
                'entity': entity,
                'top_candidates': enriched_candidates[:5]  # Store top 5 enriched candidates
            }
            ...
            if self.llm_client:
                hpo_term = self._try_llm_match(entity, enriched_candidates, original_sentence)
                if hpo_term:
                    match_info.update({
                        'hpo_term': hpo_term,
                        'match_method': 'llm',
                    ...
                    if enriched_candidates:
                        ...
                        match_info.update({
                            ...
                            'confidence_score': first_candidate.get('similarity_score', 0.1)  # Low confidence for fallback
                        })
                    else:
    ...
    def _try_llm_match(self, entity: str, candidates: List[Dict], original_sentence: Optional[str] = None) -> Optional[str]:
    ...
    def process_batch(self, entities_batch: List[List[str]], metadata_batch: List[List[Dict]], 
                     original_sentences_batch: Optional[List[List[str]]] = None) -> List[List[Dict]]:
        """Process a batch of entities for HPO term matching."""
        results = []
        ...
        for entities, metadata, original_sentences in zip(entities_batch, metadata_batch, original_sentences_batch):
...
try:
    ...
    print("Warning: rapidfuzz not found. Install with 'pip install rapidfuzz' for better performance.")
...
class OptimizedRAGHPOMatcher(BaseHPOMatcher):
    ...
    def __init__(self, embeddings_manager, llm_client=None, system_message: str = None, 
                debug: bool = False, fuzzy_threshold: int = 90, 
                max_candidates: int = 20, faiss_k: int = 800):
    ...
    def _try_llm_match(self, entity: str, candidates: List[Dict], 
                      original_sentence: Optional[str] = None) -> Optional[str]:
        ...
        with self._debug_time("LLM matching"):
            ...
            for candidate in sorted_candidates[:15]:  # Increased from 10 to 15 for better context
    ...
    def match_hpo_terms(self, entities: List[str], metadata: List[Dict], 
                       original_sentences: Optional[List[str]] = None) -> List[Dict]:
            ...
            for entity, original_sentence in zip(entities, original_sentences):
                ...
                if candidates:
                    ...
                    match_info.update({
                        'hpo_term': first_candidate['metadata']['hp_id'],
                        'match_method': 'fallback',
                        'confidence_score': min(0.5, first_candidate.get('similarity_score', 0.1))  # Cap at 0.5
                    })
                else:
    ...
    def process_batch(self, entities_batch: List[List[str]], metadata_batch: List[List[Dict]], 
                     original_sentences_batch: Optional[List[List[str]]] = None) -> List[List[Dict]]:
        """Process a batch of entities for HPO term matching."""
        with self._debug_time(f"Processing batch of {len(entities_batch)} sets of entities"):
            results = []
