#!/usr/bin/env python3
"""
Zero-shot HPO phenotype extraction baseline for the BioLark GSC benchmark.
...
Usage (from RDMA repo root):
    python baselines/biolarkgsc/zeroshot.py --model_type llama3_8b
"""
...
def main():
    parser = argparse.ArgumentParser(description="Zero-shot HPO extraction baseline for BioLark GSC")
    ...
    output = args.output or (
        _RESULTS_DIR / "biolarkgsc" / f"zeroshot_{args.model_type}_predictions.jsonl"
    )
    ...
    try:
        ...
        for i, sample in enumerate(tqdm(run_samples, total=len(run_samples), desc="BioLarkGSC-ZS")):
            try:
