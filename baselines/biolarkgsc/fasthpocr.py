#!/usr/bin/env python3
"""
FastHPOCR baseline for the BioLark GSC phenotype-mining benchmark.
...
def main():
    parser = argparse.ArgumentParser(
        description="FastHPOCR baseline on BioLark GSC phenotype-mining benchmark"
    )
    parser.add_argument(
        "--hpo_index",
        type=str,
    ...
    parser.add_argument(
        ...
        help=(
            "Output JSONL path "
            "(default: <results_dir>/biolarkgsc/fasthpocr_predictions.jsonl)"
        ),
    )
    parser.add_argument(
        "--resume",
    ...
    output = args.output or (
        _RESULTS_DIR / "biolarkgsc" / "fasthpocr_predictions.jsonl"
    )
    ...
    try:
        ...
        for i, sample in enumerate(
            tqdm(run_samples, total=len(run_samples), desc="BioLarkGSC-FastHPOCR")
        ):
            try:
