#!/usr/bin/env python3
"""
PhenoGPT baseline for the BioLark GSC phenotype-mining benchmark.
...
Usage (from RDMA repo root):
  python baselines/biolarkgsc/phenogpt.py --gpu_id 0
  python baselines/biolarkgsc/phenogpt.py --use_biosent2vec --biosent2vec_path /path/to/BioSentVec.bin
"""
...
def main():
    parser = argparse.ArgumentParser(
        description="PhenoGPT baseline on BioLark GSC phenotype-mining benchmark"
    )
    parser.add_argument(
        "--base_model",
        type=str,
    ...
    parser.add_argument(
        ...
        help=(
            "Output JSONL path "
            "(default: <results_dir>/biolarkgsc/phenogpt_predictions.jsonl)"
        ),
    )
    parser.add_argument(
        "--resume",
    ...
    output = args.output or (
        _RESULTS_DIR / "biolarkgsc" / "phenogpt_predictions.jsonl"
    )
    ...
    try:
        ...
        for i, sample in enumerate(
            tqdm(run_samples, total=len(run_samples), desc="BioLarkGSC-PhenoGPT")
        ):
            try:
