#!/usr/bin/env python3
"""
PhenoGPT2 baseline for the BioLark GSC phenotype-mining benchmark.
...
Usage (from RDMA repo root):
  python baselines/biolarkgsc/phenogpt2.py --model_dir /path/to/phenogpt2_weights
"""
...
def _extract_hpo_ids_from_output(raw_output: str) -> list:
    ...
    """
    try:
        # PhenoGPT2 outputs without the opening brace; it's prepended in inference.py
        text = "{" + raw_output if not raw_output.strip().startswith("{") else raw_output
        # Try ast.literal_eval first (more forgiving of single quotes)
        try:
...
def main():
    parser = argparse.ArgumentParser(
        description="PhenoGPT2 baseline on BioLark GSC phenotype-mining benchmark"
    )
    parser.add_argument(
        "--model_dir",
        type=str,
    ...
    parser.add_argument(
        ...
        help=(
            "Output JSONL path "
            "(default: <results_dir>/biolarkgsc/phenogpt2_predictions.jsonl)"
        ),
    )
    parser.add_argument(
        "--resume",
    ...
    output = args.output or (
        _RESULTS_DIR / "biolarkgsc" / "phenogpt2_predictions.jsonl"
    )
    ...
    # Add HPO IDs as special tokens (as in original PhenoGPT2 inference.py)
    ...
    try:
        ...
        for i, sample in enumerate(
            tqdm(run_samples, total=len(run_samples), desc="BioLarkGSC-PhenoGPT2")
        ):
            try:
