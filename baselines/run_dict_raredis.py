#!/usr/bin/env python3
"""
Run the dictionary-matching baseline on the RareDis benchmark.
...
def main():
    parser = argparse.ArgumentParser(description="Dictionary baseline on RareDis")
    parser.add_argument(
        "--embeddings_file",
        type=Path,
    ...
    parser.add_argument(
        ...
        type=Path,
        default=None,
        help="Output JSONL path (default: <results_dir>/raredis/dict_predictions.jsonl)",
    )
    parser.add_argument(
    ...
    output = args.output or (_RESULTS_DIR / "raredis" / "dict_predictions.jsonl")
    ...
    try:
        ...
        for i, sample in enumerate(tqdm(run_samples, total=len(run_samples), desc="RareDis-Dict")):
            try:
