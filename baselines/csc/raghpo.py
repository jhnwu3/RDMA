#!/usr/bin/env python3
"""
RAG-HPO baseline for the CSC phenotype-mining benchmark.
...
Usage (from RDMA repo root):
  python baselines/csc/raghpo.py
"""
...
def main():
    parser = argparse.ArgumentParser(
        description="RAG-HPO baseline on CSC phenotype-mining benchmark"
    )
    parser.add_argument(
        "--llm_type",
        type=str,
    ...
    parser.add_argument(
        ...
        help=(
            "Output JSONL path "
            "(default: <results_dir>/csc/<model_type>_raghpo_predictions.jsonl)"
        ),
    )
    parser.add_argument(
        "--resume",
    ...
    output = args.output or (
        _RESULTS_DIR / "csc" / f"{args.model_type}_raghpo_predictions.jsonl"
    )
    ...
    try:
        ...
        for i, sample in enumerate(
            tqdm(run_samples, total=len(run_samples), desc="CSC-RAGHPO")
        ):
            try:
