#!/usr/bin/env python3
"""
Retrieval + string-match Dictionary HPO baseline for the CSC benchmark.
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Dictionary HPO baseline on CSC phenotype-mining benchmark"
    )
    parser.add_argument(
        "--embeddings_file",
        type=Path,
    ...
    parser.add_argument(
        ...
        help=(
            "Output JSONL path "
            "(default: <results_dir>/csc/dictionary_hpo_predictions.jsonl)"
        ),
    )
    parser.add_argument(
    ...
    output = args.output or (_RESULTS_DIR / "csc" / "dictionary_hpo_predictions.jsonl")
    ...
    try:
        ...
        for i, sample in enumerate(
            tqdm(run_samples, total=len(run_samples), desc="CSC-DictionaryHPO")
        ):
            try:
