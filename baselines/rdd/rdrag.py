#!/usr/bin/env python3
"""
RDRAG baseline for the RDD benchmark.

Pipeline:
  1. LLM extracts rare disease mentions from the full text (no retrieval
     context).
...
Output is a JSONL file in the same format as run_rdd.py:
    {"id": "<doc_id>", "predicted": ["entity1", ...], "timing": {...}}

Usage (from RDMA repo root):
  python baselines/run_rdrag_rdd.py
"""
...
def main():
    parser = argparse.ArgumentParser(description="RDRAG baseline on RDD")
    parser.add_argument(
        "--model_type",
        type=str,
        default="qwen_32b",
    ...
    parser.add_argument(
        ...
        type=int,
        default=5,
        help="Top-k ORPHA candidates per entity (default: %(default)s)",
    )
    parser.add_argument(
        "--window_size",
        type=int,
    ...
    parser.add_argument(
        ...
        help=(
            "Output JSONL path "
            "(default: <results_dir>/rdd/<model_type>_rdrag_predictions.jsonl)"
        ),
    )
    parser.add_argument(
        "--resume",
    ...
    parser.add_argument(
        "--dev",
        action="store_true",
        help="Dev mode: process only first 2 samples",
    )
    parser.add_argument(
        "--checkpoint_interval",
        type=int,
    ...
    output = args.output or (
        _RESULTS_DIR / "rdd" / f"{args.model_type}_rdrag_predictions.jsonl"
    )
    ...
    try:
        ...
        for i, sample in enumerate(
            tqdm(run_samples, total=len(run_samples), desc="RDD-RDRAG")
        ):
            try:
