...
class RDMAVerifier:
    ...
    def _filter_hallucinated_entities(self, entities_with_contexts: List[Dict]) -> List[Dict]:
        """
        ...
        filtered_entities = [
            ...
            if entity.get("context") and len(entity.get("context", "").strip()) >= self.min_context_length 
        ...
        if removed_count > 0 and self.debug:
            self._debug_print(f"Filtered out {removed_count} potentially hallucinated entities with invalid context")
        
        return filtered_entities
    
    def _format_entities_for_verification(self, entities_with_contexts: List[Dict]) -> List[Dict]:
        """
    ...
    def verify_entities(self, entities_with_contexts: List[Dict]) -> List[Dict]:
        ...
        formatted_entities = self._format_entities_for_verification(entities_with_contexts)
        
        if self.debug:
            self._debug_print(f"Verifying {len(formatted_entities)} entities after filtering")
            
        # Skip processing if all entities were filtered out
        if not formatted_entities:
            self._debug_print("All entities were filtered out as potential hallucinations")
            return []
        ...
        if self.verifier_type == "multi_stage":
            ...
            for entity_data in formatted_entities:
                ...
                is_rare_disease = self.verifier._verify_rare_disease(entity, candidates[:self.top_k])
    ...
    def verify_from_json(self, extraction_results: Dict[str, Dict]) -> Dict[str, Dict]:
        ...
        for i, (case_id, case_data) in enumerate(extraction_results.items()):
            try:
                if self.debug:
                    self._debug_print(f"Processing case {i+1}/{total_cases} (ID: {case_id})")
                
                # Get entities with contexts
                entities_with_contexts = case_data.get("entities_with_contexts", [])
                
                if self.debug:
                    self._debug_print(f"  Processing {len(entities_with_contexts)} raw entities")
                ...
                filtered_count = original_count - len(self._format_entities_for_verification(entities_with_contexts))
    ...
    def print_verification_summary(self, results: Dict[str, Dict]):
        ...
        total_input_entities = sum(len(case_data.get("entities_with_contexts", [])) 
                                  for case_id, case_data in results.items())
        total_filtered_entities = sum(results[case_id].get("stats", {}).get("filtered_hallucinations", 0) 
                                     for case_id in results)
        total_verified_entities = sum(results[case_id].get("stats", {}).get("entities_verified", 0) 
                                     for case_id in results)
        total_rare_diseases = sum(len(results[case_id].get("verified_rare_diseases", [])) 
                                 for case_id in results)
        ...
        print(f"Filtered out as potential hallucinations: {total_filtered_entities} ({filtered_rate:.1f}%)")
        print(f"Entities after filtering: {total_verified_entities}")
        print(f"Verified as rare diseases: {total_rare_diseases} ({verification_rate:.1f}% of filtered entities)")
        print(f"Overall yield: {overall_rate:.1f}% (rare diseases from original entities)")