...
def _default_entity_system_message() -> str:
    """Default extraction system message used by entity extractors.

    This base prompt is neutral with respect to negation/family-history.
    Runtime policy controls are appended via policy override logic.
    """
    return (
        "You are a rare disease expert with extensive medical knowledge. "
        "Carefully review every sentence of the clinical passage to identify "
        "terms related to genetic inheritance patterns, anatomical anomalies, "
        "clinical symptoms, diagnostic findings, lab test results, and specific "
        "conditions or syndromes. Return the extracted terms in a JSON object "
        "with a single key 'findings', which contains the list of extracted "
        "terms spelled correctly. Ensure the output is concise without any "
        "additional notes, commentary, or meta explanations."
    )
...
def _build_extraction_recall_hints(
    ...
    Args:
        decompose_compound: If True, instruct the model to decompose named syndromes
    ...
    if decompose_compound:
        lines.append(
            ...
            "describes its component features (e.g., contractures, shortening of "
            "phalanges, atrophy), extract BOTH the syndrome name AND each described "
            "component feature as separate terms."
        )
    if extract_qualified:
        lines.append(
            "When a term has a qualifying modifier (e.g., 'conductive hearing loss', "
...
def _build_extraction_policy_override(
    negation: bool = True,
    family_history: bool = True,
) -> str:
    """Build a policy override block for extractor prompts.

    Args:
        negation: If True, exclude negated/normal findings.
            If False, allow them.
        family_history: If True, exclude family-history-only findings.
            If False, allow them.
    """
    lines = [
        "EXTRACTION POLICY OVERRIDE (highest priority):",
    ]

    if negation:
        lines.append("- Exclude negated or explicitly normal findings.")
    else:
        lines.append(
            "- Do NOT exclude negated findings; include phenotype mentions "
            "even when negated and preserve negation wording in terms."
        )

    if family_history:
        lines.append("- Exclude findings that appear only in family-history context.")
    else:
        lines.append(
            "- Do NOT exclude family-history findings; include phenotype "
            "mentions in relatives and preserve relative context in terms."
        )

    lines.append(
        "- If any prior instruction conflicts with this policy, " "follow this policy."
    )
    return "\n".join(lines)
...
class LLMEntityExtractor(BaseEntityExtractor):
    ...
    def _compose_system_message(self) -> str:
        ...
        if recall_hints:
            parts.append(recall_hints)
        parts.append(policy_override)
        return "\n\n".join(parts)

    def extract_entities(self, text: str) -> List[str]:
        findings_text = self.llm_client.query(
            text,
            self._compose_system_message(),
        )
    ...
    def extract_entities(self, text: str) -> List[str]:
        ...
        for iteration in range(self.max_iterations):
            ...
            if combined_entities == current_entities:
                print(
                    f"Early stopping at iteration {iteration+1}: No new entities found."
                )
                break
            ...
            if new_entities:
                print(
                    f"Iteration {iteration+1}: Found {len(new_entities)} new entities"
                )
            else:
...
class RetrievalEnhancedEntityExtractor(BaseEntityExtractor):
    ...
    def _retrieve_candidates(self, sentence: str) -> List[Dict]:
        ...
        with torch.no_grad():
            try:
                # Embed the query
                query_vector = self.embedding_manager.query_text(sentence).reshape(
                    1, -1
                )
                ...
                distances, indices = self.embedding_manager.search(
                    query_vector_cpu,
                    self.index,
                    k=min(800, len(self.embedded_documents)),
                )
                ...
                if "query_vector" in locals() and isinstance(
    ...
    def _create_enhanced_prompt(self, sentence: str, candidates: List[Dict]) -> str:
        ...
        # Build policy clause according to runtime extractor settings.
        policy_parts = []
        if self.negation:
            policy_parts.append("IGNORE NEGATIVE FINDINGS AND NORMAL FINDINGS")
        if self.family_history:
            policy_parts.append("IGNORE ANY TERMS MENTIONED IN FAMILY HISTORY")
        policy_clause = " AND ".join(policy_parts)

        if policy_clause:
            policy_sentence = f"{policy_clause}."
        else:
            policy_sentence = "DO NOT EXCLUDE TERMS SOLELY BECAUSE THEY ARE NEGATED OR IN FAMILY HISTORY CONTEXT."
    ...
    def extract_entities(self, text: str) -> List[str]:
        ...
        sentence_retrieve_batch_size = min(
            50, len(valid_sentences)
        )  # Process at most 50 sentences at a time for retrieval
    ...
    def _process_pairs_sequential(
        self, sentence_candidate_pairs: List[Tuple[str, List[Dict]]]
    ) -> List[str]:
        """
    ...
    def _process_pairs_batched(
        ...
        try:
            ...
            if (
                ...
                and hasattr(self.embedding_manager, "model")
                and hasattr(self.embedding_manager.model, "to")
                and isinstance(self.embedding_manager.model, SentenceTransformer)
            ):
            ...
            for i in range(0, len(sentence_candidate_pairs), self.max_batch_size):
                ...
                system_messages = [self._compose_system_message()] * len(prompts)

                # Process this chunk in batch
                with torch.no_grad():  # Use no_grad for LLM generation if it uses tensors
                    batch_responses = self.llm_client.batched_query(
...
class NuExtractor(BaseEntityExtractor):
    ...
    def _initialize_model(self):
        ...
        try:
            self.tokenizer = AutoTokenizer.from_pretrained(
                self.model_name, trust_remote_code=False, cache_dir=self.cache_dir
            )
    ...
    def extract_entities(self, text: str) -> List[str]:
        ...
        try:
            ...
            extraction_text = (
                output.split("<|output|>")[1].split("<|end-output|>")[0].strip()
            )
    ...
    def _process_extraction(self, extraction: Dict) -> List[str]:
        ...
        filtered_findings = [
            ...
            if (
                finding.get("mention")
                and not finding.get("negated", False)
                and not finding.get("family_history", False)
            )
        ]
