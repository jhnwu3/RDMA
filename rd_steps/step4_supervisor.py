...
def parse_arguments() -> argparse.Namespace:
    ...
    parser = argparse.ArgumentParser(description="Supervise evaluation results for rare disease matching")
...
def load_data(predictions_file: str, ground_truth_file: str, evaluation_file: str) -> Tuple[Dict, Dict, Dict]:
    """
...
def extract_entities_with_context(
                ...
                if isinstance(gt_sample, dict) and 'annotations' in gt_sample:
                    for ann in gt_sample['annotations']:
                        ...
                        if ordo_field.startswith(code) or code in ordo_field:
                            ...
                            document_text = gt_sample.get('note_details', {}).get('text', '')
                            
                            # Extract context around the entity if possible
                            if document_text and gt_entity and gt_entity in document_text:
                                # Extract a simple context window
                                pos = document_text.find(gt_entity)
                                start = max(0, pos - 100)
                                end = min(len(document_text), pos + len(gt_entity) + 100)
    ...
    for category, eval_key in [('false_positives', 'corpus_false_positives'), 
...
def process_entities(
    ...
    for category in ['false_negatives', 'false_positives', 'true_positives']:
        ...
        for i, entity_data in enumerate(tqdm(category_entities, desc=f"Verifying {category}")):
            try:
                ...
                timestamp_print(f"Error processing {category} entity '{entity_data.get('entity', '')}': {e}")
                if args.debug:
                    traceback.print_exc()
...
def prepare_summary(verification_results: Dict[str, List[Dict]]) -> Dict[str, Any]:
    ...
    for category in ['false_negatives', 'false_positives', 'true_positives']:
        ...
        if category_stats['total'] > 0:
            ...
            category_stats['confirmed_rare_disease_percentage'] = is_rare_counts['YES'] / category_stats['total'] * 100
            
            category_stats['flagged_for_review_count'] = flag_for_review_counts['YES']
            category_stats['flagged_for_review_percentage'] = flag_for_review_counts['YES'] / category_stats['total'] * 100
    ...
    summary['total_entities'] = sum(summary['categories'][cat]['total'] for cat in summary['categories'])
    summary['total_flagged_for_review'] = len(all_flagged_for_review)
    summary['flagged_for_review_percentage'] = (summary['total_flagged_for_review'] / summary['total_entities'] * 100) if summary['total_entities'] > 0 else 0.0
...
def main():
    ...
    try:
        ...
        timestamp_print(f"Total entities flagged for review: {summary['total_flagged_for_review']} "
        ...
        for category in ['false_negatives', 'false_positives', 'true_positives']:
            ...
            timestamp_print(f"  Confirmed as rare disease: {stats['confirmed_rare_disease_count']} "
                          f"({stats['confirmed_rare_disease_percentage']:.1f}%)")
            timestamp_print(f"  Flagged for review: {stats['flagged_for_review_count']} "
