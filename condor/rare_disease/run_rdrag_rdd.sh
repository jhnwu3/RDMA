#!/bin/bash
# run_rdrag_rdd.sh — runs one RDRAG RDD baseline job under HTCondor
...
echo "[$(date)] Starting rdrag_rdd  model_type=$1"
python baselines/rdd/rdrag.py --condor --model_type "$1"
echo "[$(date)] Finished rdrag_rdd  model_type=$1"
