    ...
    def extract_entities(self, text: str) -> List[str]:
        ...
        for iteration in range(self.max_iterations):
            ...
            if combined_entities == current_entities:
                print(f"Early stopping at iteration {iteration+1}: No new rare disease entities found.")
                break
                
            # Calculate new entities found in this iteration
            new_entities = combined_entities - current_entities
            if new_entities:
                print(f"Iteration {iteration+1}: Found {len(new_entities)} new rare disease entities")
            else:
